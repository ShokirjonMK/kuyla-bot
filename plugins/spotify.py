...
class SpotifyDownloader:
    ...
    async def extract_data_from_spotify_link(event, spotify_url):
        ...
        try:
            ...
            await event.respond("Spotify havolasini qayta ishlashda xatolik yuz berdi. Iltimos, qayta urinib koʻring.")
            return {}
    ...
    async def download_and_send_spotify_info(event, is_query: bool = True) -> bool:
        ...
        if not await db.get_user_updated_flag(user_id):
            await event.respond(
                "Bizning bot yangilandi. Iltimos, /start buyrug'i bilan botni qayta ishga tushiring."
            )
            return True
        ...
        if link_info["type"] == "track":
            ...
            await event.respond(
                f"""Qo'llab-quvvatlanmaydigan Spotify havolasi turi.\n\nHozirda bot qo'llab-quvvatlanadi:\n- track \n- playlist\n\nYou 
                so'ragan: {link_info["type"]} """)
            return False
    ...
    async def send_playlist_info(client, event, link_info):
        ...
        playlist_info = (
            ...
            f"  - 👥 Kuzatuvchilar: {followers}\n"
            f"  - 🎵 Umumiy treklar: {total_tracks}\n"
            f"  - 🤝 Hamkorlik: {collaborative}\n"
            f"  - 🌐 Ochiq: {public}\n"
            f"  - 🎧 Playlist manzili: [Spotify’da tinglash]({playlist_url})\n"
            f"---\n\n"
            f"**Musiqadan zavqlaning!** 🎶"
        )

        # Interaktiv tugmalar
        buttons = [
            [Button.inline("Barcha treklarni yuklab olish [mp3]", data=f"spotify/dl/playlist/{playlist_id}/all")],
            [Button.inline("Top 10-ni yuklab olish", data=f"spotify/dl/playlist/{playlist_id}/10")],
            [Button.inline("Treklarni izlash", data=f"spotify/s/playlist/{playlist_id}")],
    ...
    async def send_track_info(client, event, link_info):
        ...
        SpotifyInfoButtons = [
            [Button.inline("30 soniyali prev'yuni yuklab olish",
                        data=f"spotify/dl/30s_preview/{link_info['preview_url'].split('?cid')[0].replace('https://p.scdn.co/mp3-preview/', '')}")
            if link_info['preview_url'] is not None else Button.inline("30 soniyali prev'yuni yuklab olish",
                                                                        data=b"unavailable_feature")],
            [Button.inline("Trekni yuklab olish", data=f"spotify/dl/music/{link_info['track_id']}")],
            [Button.inline("Ikonkani yuklab olish",
                        data=f"spotify/dl/icon/{link_info['image_url'].replace('https://i.scdn.co/image/', '')}")],
            [Button.inline("Artist haqida ma'lumot", data=f"spotify/artist/{link_info['track_id']}")],
            [Button.inline("Qo'shiq matni", data=f"spotify/lyrics/{link_info['track_id']}")],
            [Button.url("Spotify’da tinglash", url=link_info["track_url"]),
            Button.url("YouTube’da tinglash", url=link_info['youtube_link']) if link_info['youtube_link'] else Button.inline("YouTube’da tinglash", data=b"unavailable_feature")],
            [Button.inline("Bekor qilish", data=b"cancel")]
        ]

        caption = (
            f"**🎧 Nom:** [{link_info['track_name']}]({link_info['track_url']})\n"
            f"**🎤 Ijrochi:** [{link_info['artist_name']}]({link_info['artist_url']})\n"
            f"**💽 Albom:** [{link_info['album_name']}]({link_info['album_url']})\n"
            f"**🗓 Chiqarilgan yil:** {link_info['release_year']}\n"
            f"**❗️ Mahalliy:** {is_local}\n"
            f"**🌐 ISRC:** {link_info['isrc']}\n"
            f"**🔄 Yuklab olingan:** {await db.get_song_downloads(link_info['track_name'])} marta\n\n"
            f"**Rasm manzili:** [Bu yerda bosing]({link_info['image_url']})\n"
    ...
    async def send_local_file(event, file_info, spotify_link_info, is_playlist: bool = False) -> bool:
        ...
        if was_local and not is_playlist:
            local_availability_message = await event.respond(
                "Ushbu musiqa topildi. Siz uchun hozir tayyorlanyapti...")

        # Provide feedback to the user during the upload process
        if not is_playlist:
            upload_status_message = await event.reply("Yuklanmoqda... Iltimos, kuting.")
        ...
        try:
            ...
            await event.respond(f"Afsuski, yuklash amalga oshmadi.\nSababi: {e}") if not is_playlist else None
    ...
    async def _upload_file(event, file_info, spotify_link_info, playlist: bool = False):
        ...
        await event.client.send_file(
            ...
            media,
            caption=(
                f"🎵 **{spotify_link_info['track_name']}** ijro etuvchi: **{spotify_link_info['artist_name']}**\n\n"
                f"▶️ [Spotify’da tinglash]({spotify_link_info['track_url']})\n"
                + (f"🎥 [YouTube’da tomosha qilish]({video_url})\n" if video_url else "")
            ),
    ...
    async def download_spotdl(event, music_quality, spotify_link_info, quite: bool = False, initial_message=None,
        ...
        if initial_message is None and not quite:
            ...
            initial_message = await event.reply("SpotDL: Yuklab olinmoqda...\nYondashuv\nApproach: Piped")
        elif quite:
            ...
            while True:
                ...
                if not quite:
                    if audio_option == "piped":
                        await message.edit(f"SpotDL: Yuklab olinmoqda...\nYondashuv\nApproach: Piped\n\n{line}")
    ...
    async def download_spotify_file_and_send(event) -> bool:
        ...
        if await db.get_file_processing_flag(user_id):
            await event.respond("Kechirasiz, siz uchun fayl allaqachon qayta ishlanmoqda.")
            return True

        fetch_message = await event.respond("Maʼlumot olinmoqda... Iltimos, kuting.")
    ...
    async def download_playlist(event, spotify_link_info, number_of_downloads: str):
        ...
        start_message = await event.respond("Pleylist tekshirilmoqda ....")

        batch_size = 10
        track_batches = [tracks_info[i:i + batch_size] for i in range(0, len(tracks_info), batch_size)]
        download_tasks = []

        await start_message.edit("Musiqalar yuborilmoqda.... Iltimos, kuting.")
    ...
    async def send_artists_info(event):
        ...
        message = "🎤 <b>San'atkorlar haqida ma'lumot</b> :\n\n"
        for artist in artist_details:
            message += f"🌟 <b>San'atkor ismi:</b> {artist['name']}\n"
            message += f"👥 <b>Kuzatuvchilar:</b> {artist['followers']}\n"
            message += f"🎵 <b>Janrlar:</b> {', '.join(artist['genres'])}\n"
            message += f"📈 <b>Ommaboplik:</b> {artist['popularity']}\n"
            if artist['image_url']:
                message += f"\n🖼️ <b>Rasm:</b> <a href='{artist['image_url']}'>Rasm manzili</a>\n"
            message += f"🔗 <b>Spotify havolasi:</b> <a href='{artist['external_url']}'>Spotify havolasi</a>\n\n"
            message += "───────────\n\n"
        ...
        artist_buttons.append([Button.inline("O'chirish", data='cancel')])
    ...
    async def send_music_lyrics(event):
        ...
        waiting_message = await event.respond("Qo'shiq matnini qidirilmoqda ... ....")
        ...
        if song:
            ...
            if not lyrics:
                error_message = "Afsuski, ushbu musiqa uchun matn topilmadi."
    ...
    async def send_music_icon(event):
        try:
            ...
            await event.reply("So'rovingizni qayta ishlashda xatolik yuz berdi. Iltimos, keyinroq qayta urinib ko'ring.")
