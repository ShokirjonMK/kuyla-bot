...
class YoutubeDownloader:
    ...
    async def send_youtube_info(client, event, youtube_link):
        ...
        try:
            await client.send_file(
                event.chat_id,
               file=thumbnail,
               caption="Yuklash uchun formatni tanlang:",
               buttons=buttons
               )
    ...
    async def download_and_send_yt_file(client, event):
        ...
        if await db.get_file_processing_flag(user_id):
            return await event.respond("Kechirasiz, siz uchun fayl allaqachon qayta ishlanmoqda.")
        ...
        if len(parts) == 6:
            ...
            if float(filesize) > YoutubeDownloader.MAXIMUM_DOWNLOAD_SIZE_MB:
                return await event.answer(
                    f"⚠️ Fayl hajmi {YoutubeDownloader.MAXIMUM_DOWNLOAD_SIZE_MB}MB dan katta."
                    , alert=True)
            ...
            if not os.path.isfile(path):
                downloading_message = await event.respond("Siz uchun fayl yuklab olinmoqda ...")
                ydl_opts = {
                    'format': format_id,
                    'outtmpl': path,
                    'quiet': True,
                ...
                local_availability_message = await event.respond(
                    "Ushbu fayl mavjud. Siz uchun hozir tayyorlanmoqda...")
            ...
            try:
                ...
                async with client.action(event.chat_id, 'document'):
                    ...
                    if extension == "mp4":
                        ...
                        audio_attributes = DocumentAttributeAudio(
                            ...
                            performer="@KuylaydiBot",  # Replace with actual performer
                            # Add other attributes as needed
                        )
                ...
                await db.set_file_processing_flag(user_id, is_processing=False)
                return await event.respond(f"Kechirasiz, so'rovingizda muammo yuz berdi.\Sababi:{str(Err)}")
        else:
            await event.answer("Tugma maʼlumotlari notoʻgʻri.")
