...
class Insta:
    ...
    async def download_content(client, event, start_message, link) -> bool:
        ...
        try:
            if content_type == 'reel':
                ...
                await event.reply(
                    "Kechirasiz, soʻralgan kontentni topib boʻlmadi. Iltimos, u hamma uchun ochiq ekanligiga ishonch hosil qiling.")
                await start_message.delete()
                return True
        except:
            await event.reply("Kechirasiz, soʻralgan kontentni topib boʻlmadi. Iltimos, u hamma uchun ochiq ekanligiga ishonch hosil qiling.")
            await start_message.delete()
            return False
    ...
    async def download(client, event) -> bool:
        ...
        start_message = await event.respond("Sizning insta havolangizga ishlov berilmoqda...")
        try:
    ...
    async def download_reel(client, event, link):
        ...
        if content_value:
            ...
            await event.reply("Nimadir xato ketdi")
    ...
    async def send_file(client, event, content_value):
        try:
            await client.send_file(event.chat_id, content_value, caption="Mana sizning Instagram kontentingiz")
        except:
            fileoutput = f"{str(content_value)}"
            downfile = wget.download(content_value, out=fileoutput)
            await client.send_file(event.chat_id, fileoutput, caption="Mana sizning Instagram kontentingiz")
