...
class X:
    ...
    async def take_screenshot_of_tweet(event, tweet_url):
        tweet_message = await event.respond(
            "Sizning so'rovingiz ko'rib chiqilmoqda... ...\nIltimos, skrinshot yaratilguncha kuting.\nBu ozroq vaqt talab qilishi mumkin ")
        ...
        try:
            ...
            await tweet_message.edit(
                f"Noqulaylik uchun uzr so'raymiz.\nSo'ralgan tvit topilmadi. Sababi: {str(Err)}")
            return None
    ...
    async def send_screenshot(client, event, tweet_url) -> bool:
        ...
        screen_shot_message = await event.respond("Skrinshot yuklanmoqda. Iltimos, turing...")

        button = Button.inline("Media yuklab olish",
        ...
        try:
            await client.send_file(
                event.chat_id,
                screenshot_path,
                caption="Mana so'ralgan tvit skrinshoti.",
                buttons=button
            )
        except Exception as Err:
    ...
    async def download(client, event):
        ...
        if media_url:
            try:
                upload_message = await event.reply("Media yuklanmoqda... Iltimos, kuting.")
                await client.send_file(event.chat_id, media_url,
                                    caption="Foydalanganingiz uchun rahmat - @KuylaydiBot")
                await upload_message.delete()
            except Exception as e:
                print(f"Faylni jo'natishda xatolik: {e}")
                await event.reply("Oops! Noto'g'ri havola yoki media mavjud emas :(")
        else:
            await event.reply("Oops! Noto'g'ri havola yoki media mavjud emas :(")
